...
const QT_OPTIONS = [
  ...
  `Keep building, keep believing ??? ${QT_HASHTAG}`,
...
type SharedUiConfig = {
  buttons: Record<string, { enabled: boolean; targetType?: string; pagePath?: string }>;
};
...
export default function Home() {
  ...
  const [nftLeftImages, setNftLeftImages] = useState<string[]>(["/shared/thumb-1.svg", "/shared/thumb-2.svg", "/shared/thumb-3.svg", "/shared/thumb-4.svg", "/shared/thumb-5.svg"]);
  const [nftRightImages, setNftRightImages] = useState<string[]>([]);
  const [calendarIcon, setCalendarIcon] = useState("/shared/thumb-1.svg");
  const [durationIcon, setDurationIcon] = useState("/shared/thumb-1.svg");
  const [salePhases, setSalePhases] = useState([
    { label: 'Presale 1', start: 'Feb 22, 2026', duration: '7 Days', mintLimit: '2 per wallet', price: '0.06 SOL' },
    { label: 'Presale 2', start: 'Mar 1, 2026',  duration: '9 Days', mintLimit: '2 per wallet', price: '0.12 SOL' },
    { label: 'Presale 3', start: 'Mar 10, 2026', duration: '5 Days', mintLimit: '2 per wallet', price: '0.15 SOL' },
    { label: 'Presale 4', start: 'Mar 15, 2026', duration: '4 Days', mintLimit: '2 per wallet', price: '0.18 SOL' },
    { label: 'Public Sale', start: 'Mar 20, 2026', duration: '10 Days', mintLimit: '5 per wallet', price: '0.22 SOL' },
  ]);
  const [connectBehavior, setConnectBehavior] = useState<"REGISTERED_WALLETS" | "REAL_WALLET_REQUIRED">("REGISTERED_WALLETS");
  const [connectBehaviorBadgeVisible, setConnectBehaviorBadgeVisible] = useState(true);
  ...
  type ConnectBehavior = "REGISTERED_WALLETS" | "REAL_WALLET_REQUIRED";

  const normalizeConnectBehavior = (raw?: string): ConnectBehavior => {
    const value = (raw || "").toUpperCase().trim();
    if (value === "REAL_WALLET_REQUIRED") return "REAL_WALLET_REQUIRED";
  ...
  const isPreviewMode = (): boolean => {
    return connectBehavior === "REGISTERED_WALLETS";
  };
  ...
  const loadRuntimeCache = () => {
    try {
      ...
      if (cached) {
        ...
        if (data.connectBehavior) {
          setConnectBehavior(normalizeConnectBehavior(data.connectBehavior));
        }
        const badgeVisible = data.connectBehaviorBadgeVisible;
        if (badgeVisible !== undefined) setConnectBehaviorBadgeVisible(!!badgeVisible);
  ...
  const loadPublicRuntime = async () => {
    try {
      ...
      const checknftsData = result.checknfts as Record<string, unknown> | null;
      if (data) {

        const behavior = normalizeConnectBehavior(data.connectBehavior as string);
        setConnectBehavior(behavior);
        ...
        saveRuntimeCache({
          ...
          connectBehaviorBadgeVisible: badgeVisible ?? true,
          ctaVisibility: ctaVisibility,
        });
      }

      // Images & checknfts data load independently of homepage config
      if (imagesData) {
        if (imagesData.checknftsLeft) setNftLeftImages(imagesData.checknftsLeft as string[]);
        if (imagesData.checknftsRight) setNftRightImages(imagesData.checknftsRight as string[]);
        if (imagesData.calendarIcon) setCalendarIcon(imagesData.calendarIcon as string);
        if (imagesData.durationIcon) setDurationIcon(imagesData.durationIcon as string);
        const imgs = {
      ...
      if (checknftsData?.salePhases) setSalePhases(checknftsData.salePhases as typeof salePhases);
  ...
  const checkDeviceSubmissionLimit = async () => {
    const deviceId = getDeviceId();
    const today = getTodayDate();

    try {
      const deviceData = await api.get(`device_submissions/${deviceId}`);

      if (deviceData === null) {
        return { allowed: true, reason: 'first_time' };
      }

      const firstDate = deviceData.first_date;
      const dailyData = deviceData.daily || {};
      const todayCount = dailyData[today] || 0;
      const totalCount = deviceData.total || 0;

      const firstDateObj = new Date(firstDate);
      const todayObj = new Date(today);
      const daysSinceFirst = Math.floor((todayObj.getTime() - firstDateObj.getTime()) / (1000 * 60 * 60 * 24));

      if (daysSinceFirst === 0) {
        if (todayCount >= 3) {
          return { allowed: false, reason: 'limit_3_per_day', remaining: 0 };
        }
        return { allowed: true, reason: 'first_day', remaining: 3 - todayCount };
      } else {
        if (todayCount >= 1) {
          return { allowed: false, reason: 'limit_1_per_day', remaining: 0 };
        }
        return { allowed: true, reason: 'after_first_day', remaining: 1 - todayCount };
      }
    } catch (error) {
  ...
  const updateDeviceSubmission = async () => {
    const deviceId = getDeviceId();
    const today = getTodayDate();
    ...
    try {
      ...
      if (deviceData === null) {
        await api.set(`device_submissions/${deviceId}`, {
          total: 1,
          daily: {
            [today]: 1
          },
        ...
        const currentTotal = deviceData.total || 0;
        const dailyData = deviceData.daily || {};
        const todayCount = dailyData[today] || 0;

        const newDaily = { ...dailyData };
        newDaily[today] = todayCount + 1;

        await api.update(`device_submissions/${deviceId}`, {
  ...
  const updateReferralCount = async (inviterUsername: string, referredUserKey: string, referredUsername: string) => {
    if (!inviterUsername || !referredUserKey) return;
    ...
    try {
      ...
      const inviterName = inviterUsername.replace("@", "").toLowerCase();

      for (const key in users) {
        const storedTwitter = users[key].username?.replace("@", "").toLowerCase();
        if (storedTwitter !== inviterName) continue;

        const inviterDeviceId = users[key].device_id;

        if (inviterDeviceId === currentDeviceId) {
          console.log("Same device referral - not counting towards referrals");
          return;
        }

        const existingEvent = await api.get(`referralEvents/${referredUserKey}`);
        if (existingEvent !== null) {
          return;
        }

        await api.set(`referralEvents/${referredUserKey}`, {
          inviter: inviterUsername,
          inviter_user_key: key,
          referredUsername,
          device_id: currentDeviceId,
          createdAt: Date.now(),
        });

        const perReferral = mxpRewards.perReferral || 50;

        const currentUser = await api.get(`users/${key}`);
        if (currentUser) {
          const newCount = (currentUser.referrals || 0) + 1;
          const newStatus = getStatusFromReferrals(newCount);
          const baseMxp = typeof currentUser.mxp === "number" ? currentUser.mxp : 0;

          const newMxp = baseMxp + perReferral;
          const newMxpFromReferrals = (currentUser.mxpFromReferrals || 0) + perReferral;
          ...
          const updated = {
            ...
            status: newStatus,
            mxp: newMxp,
            mxpFromReferrals: newMxpFromReferrals,
            lastUpdated: new Date().toISOString(),
          };

          await api.update(`users/${key}`, updated);

          if (updated) {
            const existing = cachedUsers.get(storedTwitter);
            setCachedUsers((prev) =>
              new Map(
                prev.set(storedTwitter, {
                  ...existing,
                  referrals: updated.referrals || 0,
                  status: updated.status || statusNames.default,
                  wallet: updated.wallet || existing?.wallet || "",
                  comment1: updated.comment_1 || existing?.comment1 || "",
                  comment2: updated.comment_2 || existing?.comment2 || "",
                  comment3: updated.comment_3 || existing?.comment3 || "",
                  mxp: updated.mxp || 0,
                  mxpFromUsername: updated.mxpFromUsername || existing?.mxpFromUsername || 0,
                  mxpFromInvitee: updated.mxpFromInvitee || existing?.mxpFromInvitee || 0,
                  mxpFromReferrals: updated.mxpFromReferrals || existing?.mxpFromReferrals || 0,
                  mxpFromTasks: updated.mxpFromTasks || existing?.mxpFromTasks || 0,
                  reviewStatus: normalizeReviewStatus(updated.reviewStatus),
                  fakeMxpBlocked:
                    normalizeReviewStatus(updated.reviewStatus) === "NEEDS_IMPROVEMENT" ||
                    normalizeReviewStatus(updated.reviewStatus) === "DISQUALIFIED",
                })
              )
            );
          }
      ...
      {startupOverlayVisible && (
        <div className="startup-overlay">
          <div className="startup-overlay-content">
            <img src="/logo.PNG" alt={SITE.projectName} className="startup-logo" />
          ...
          {loggedInUsername ? (
            <>
                  ...
                  <div className="user-card-top">
                    <div className="profile-box">
                      <div className="user-info-left">
                        <img src="/logo.PNG" alt={SITE.projectName} className="user-logo-icon" />
              ...
              <div className="action-buttons homepage-actions">
                  <button
                    className="btn primary-btn"
                    onClick={() => window.open('https://opensea.io/collection/ethereum-apes-nft/overview')}
                  >
                    MINT ON OPENSEA
                  </button>
                  {salePhases.map((p, i) => (
                    <div key={i} className={`sale-phase-card sale-phase-${i + 1}`}>
                       <div className="sale-phase-art-box" aria-hidden="true">
                        <img src={(i < 4 ? nftLeftImages[i] : nftRightImages[0]) || calendarIcon} alt={p.label} style={{ width: "100%", height: "100%", objectFit: "cover" }} />
                      </div>

                      <div className="sale-phase-main">
                        <div className="sale-phase-heading-row">
                          <div className="sale-phase-title">{p.label}</div>
                          <div className="sale-phase-mint-limit">Mint Limit: {p.mintLimit}</div>
                        </div>

                        <div className="sale-phase-details sale-phase-compact-details">
                          <div className="sale-phase-detail">
                            <span className="sale-phase-icon">
                              <img src={calendarIcon} alt="" style={{ width: 20, height: 20, borderRadius: "50%", objectFit: "cover" }} />
                            </span>
                            <span className="sale-phase-copy">
                              <span className="sale-phase-label">Start Date</span>
                              <strong>{p.start}</strong>
                            </span>
                          </div>
                          <div className="sale-phase-detail">
                            <span className="sale-phase-icon">
                              <img src={durationIcon} alt="" style={{ width: 20, height: 20, borderRadius: "50%", objectFit: "cover" }} />
                            </span>
                            <span className="sale-phase-copy">
                              <span className="sale-phase-label">Duration</span>
                      ...
                      <div className="sale-phase-price-box">
                        <span className="sale-phase-price-label">Price</span>
                        <span className="sale-phase-price-value">
                          <img
                            src={nftRightImages[1] || nftRightImages[0] || "/shared/icon-top-1.svg"}
                            alt=""
                            style={{ width: 24, height: 24, borderRadius: 4, objectFit: "cover", border: "2px solid #1E1E1E", padding: 2, background: "#FFF" }}
                          />
                          {p.price}
                        </span>
                      </div>
            ...
            <>
              ...
              <div className="entry-box">
                <div className="entry-box-header">
                  <img src="/logo.PNG" alt={SITE.projectName} className="entry-box-logo" />
      ...
      {currentStep === "join" && (
        <div className="step-screen join-entry-screen">
          <div className="join-entry-header">
            <img src="/logo.PNG" alt={SITE.projectName} className="join-entry-logo" />
            ...
            <>
              ...
              <div className="mxp-token-section">
                <div className="mxp-token-header">
                  <img src="/logo.PNG" alt={SITE.xpLabel} className="mxp-token-logo" />
