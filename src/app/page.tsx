  ...
  const updateReferralCount = async (inviterUsername: string, referredUserKey: string, referredUsername: string) => {
    ...
    try {
      ...
      for (const key in users) {
        ...
        const current = await api.get(`users/${key}`);
        if (current) {
          const newCount = (current.referrals || 0) + 1;
          const newStatus = getStatusFromReferrals(newCount);
          const baseMxp = typeof current.mxp === "number" ? current.mxp : 0;

          const newMxp = baseMxp + perReferral;
          const newMxpFromReferrals = (current.mxpFromReferrals || 0) + perReferral;
          ...
          await api.set(`users/${key}`, updated);
